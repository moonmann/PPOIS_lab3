...
def play_loop(clock, screen, sounds, buttons, cursor, cursor_group, chickens_small_group, chickens_mid_group,
              chickens_big_group, green_chickens_group, ammo, ammo_group, score_manager, scores_group, pumpkin,
              sign_post, big_chicken_group, mill):
    # background SOUND
    ...
    while running:
        ...
        if cursor_x >= 750 and cursor_x <= 800:
            if camera1.move(2) and camera2.move(5) and camera3.move(15) and camera4.move(40):
                ...
                green_chickens_group.update(dt, 'move_r')
            ...
            if camera1.move(-2) and camera2.move(-5) and camera3.move(-15) and camera4.move(-40):
                ...
                green_chickens_group.update(dt, 'move_l')
        ...
        for event in pygame.event.get():
            ...
            # add new CHICKEN on the screen0
                ...
                green_chickens_group.add(ChickenGreen(screen, randint(100, 300)))
            ...
            # checks if we have shot a CHICKEN
                ...
                # if we shot the CHICKEN
                    ...
                    print('small')
                    break
                elif cursor.shoot_green_chicken(sounds, green_chickens_group, check_shot, score_manager, scores_group):
                    print('green')
                    break
        ...
        # update FLY CHICKEN GREEN
        green_chickens_group.draw(screen)
        green_chickens_group.update(dt, 'no')
        screen.blit(green, (-camera4.rect[0], camera4.rect[1]))
